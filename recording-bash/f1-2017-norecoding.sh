...
#else
  ffmpeg  -y -video_size "$W"x"$H" -framerate "$FPS" -f x11grab -i "$CURRENT_DISPLAY_ID".0+"$X","$Y" -c:v "$CODEC" -vf "scale=$SCALE_TO[a]; [a]crop=$CROP" -crf "$QUALITY" -preset "$PRESET" "$OUT_FILENAME".mkv
#fi
